...
namespace DevNest.Business.Domain.Domains.VaultX
{
    ...
    public class VaultXStoreDomainService(
        IAppLogger<VaultXStoreDomainService> logger,
        IVaultXStoreRepositoryRouter router,
        IAppConfigService<VaultXConfigurationsEntityModel> applicationConfigService,
        IMapper mapper) : IVaultXStoreDomainService
    {
        private readonly IAppLogger<VaultXStoreDomainService> _logger = logger;
        private readonly IVaultXStoreRepositoryRouter _router = router;
            ...
            SearchEntityModel? searchFilter)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Get)} method called.");

                var data = (await _router.GetAsync(workspace))?.ToList();
                if (data == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Get)} method returned null data.");
                ...
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Get)} method returned {data?.Count} credentials.");
            ...
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {ex.Message}", ex);
        ...
        public async Task<AppResponse<CredentialResponseDTO>> GetById(Guid id, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(GetById)} method called with ID: {id}.");
                var entity = await _router.GetByIdAsync(id, workspace);
                if (entity == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(GetById)} method returned null for ID: {id}.");
                ...
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(GetById)} method returned credentials for ID: {id}.");
        ...
        public async Task<AppResponse<bool>> Delete(string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Delete)} method called to delete all credentials.");
                var result = await _router.DeleteAsync(workspace);
                if (result == false)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Delete)} method failed to delete all credentials.");
                    return new AppResponse<bool>(Messages.GetError(ErrorConstants.DeleteCredentialsFailed_All));
                }
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Delete)} method successfully deleted all credentials.");
        ...
        public async Task<AppResponse<bool>> DeleteById(Guid id, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(DeleteById)} method called with ID: {id}.");
                var result = await _router.DeleteByIdAsync(id, workspace);
                if (result == false)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(DeleteById)} method failed to delete credential with ID: {id}.");
                    return new AppResponse<bool>(Messages.GetError(ErrorConstants.DeleteCredentialsFailed_ById));
                }

                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(DeleteById)} method successfully deleted credential with ID: {id}.");
                return new AppResponse<bool>(
                    data: result,
        ...
        public async Task<AppResponse<CredentialResponseDTO>> Add(AddCredentialRequest? request, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Add)} method called with request: {request}.");
                ...
                if (result == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Add)} method failed to create credentials.");
                    return new AppResponse<CredentialResponseDTO>(Messages.GetError(ErrorConstants.CreateCredentialsFailed)) { Data = null };
                }

                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Add)} method successfully created credentials with ID: {result.Id}.");
        ...
        public async Task<AppResponse<CredentialResponseDTO>> Update(UpdateCredentialRequest? request, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Update)} method called with request: {request}.");
                var entity = _mapper.Map<CredentialEntityModel>(request);
                ...
                if (result == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Update)} method failed to update credentials.");
                    return new AppResponse<CredentialResponseDTO>(Messages.GetError(ErrorConstants.UpdateCredentialsFailed));
                }

                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Update)} method successfully updated credentials with ID: {result.Id}.");
                return new AppResponse<CredentialResponseDTO>(
                    data: _mapper.Map<CredentialResponseDTO>(result),
                    message: string.Format(
        ...
        public async Task<AppResponse<bool>> Archive(Guid id, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Archive)} method called with ID: {id}.");
                var result = await _router.ArchiveByIdAsync(id, workspace);
                if (result == false)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Archive)} method failed to archive credential with ID: {id}.");
                    return new AppResponse<bool>(Messages.GetError(ErrorConstants.DeleteCredentialsFailed_ById));
                }

                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Archive)} method successfully archived credential with ID: {id}.");
                return new AppResponse<bool>(
                    data: result,
                    message: string.Format(
        ...
        public async Task<AppResponse<CredentialResponseDTO>> Encrypt(Guid id, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Encrypt)} method called with ID: {id}.");
                var entity = await _router.EncryptByIdAsync(id, workspace);
                if (entity == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Encrypt)} method returned null for ID: {id}.");
                ...
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Encrypt)} method returned credentials for ID: {id}.");
        ...
        public async Task<AppResponse<CredentialResponseDTO>> Decrypt(Guid id, string workspace)
        {
            try
            {
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Decrypt)} method called with ID: {id}.");
                var entity = await _router.DecryptByIdAsync(id, workspace);
                if (entity == null)
                {
                    _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Decrypt)} method returned null for ID: {id}.");
                ...
                _logger.LogDebug($"{nameof(VaultXStoreDomainService)} => {nameof(Decrypt)} method returned credentials for ID: {id}.");
        ...
        private void PerformTextSearch(ref List<CredentialEntityModel>? entity, TextSearchEntityModel searchEntity)
        {
            ...
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformTextSearch)}: Field name is null or empty.");
                throw new ArgumentException("Text search filter requires a valid field name.");
            }

            if (string.IsNullOrWhiteSpace(comparison))
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformTextSearch)}: Comparison type is null or empty.");
                throw new ArgumentException("Text search filter requires a valid comparison type.");
            }

            if (string.IsNullOrWhiteSpace(value))
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformTextSearch)}: Comparison value is null or empty.");
            ...
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformTextSearch)}: Invalid or unsupported field '{fieldName}' for text search.");
            ...
            switch (comparison.ToLower())
            {
                ...
                default:
                    _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformTextSearch)}: Unsupported comparison type '{comparison}'.");
        ...
        private void PerformDateSearch(ref List<CredentialEntityModel>? entity, DateSearchEntityModel searchEntity)
        {
            ...
            if (string.IsNullOrWhiteSpace(fieldName))
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformDateSearch)}: Field name is null or empty.");
            ...
            {
                _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformDateSearch)}: Invalid or unsupported datetime field '{fieldName}'.");
            ...
            switch (comparison?.ToLower())
            {
                ...
                default:
                    _logger.LogError($"{nameof(VaultXStoreDomainService)} => {nameof(PerformDateSearch)}: Unsupported date comparison type '{comparison}'.");
