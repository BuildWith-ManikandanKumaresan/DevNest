...
namespace DevNest.Encryption.Plugin.AES.GCM
{
    ...
    public class EncryptionContext<T>(Dictionary<string, object>? _connectionParams, IAppLogger<EncryptionPlugin> logger) : IEncryptionContext<T> where T : class
    {
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        {
            _logger.LogDebug($"{nameof(EncryptionContext<T>)} => {nameof(Decrypt)} called with cipherText: {cipherText}");
...
#pragma warning disable SYSLIB0053 // Type or member is obsolete
            using (var aesGcm = new AesGcm(SHA256.HashData(Encoding.UTF8.GetBytes(_key))))
            {
                ...
                _logger.LogDebug($"{nameof(EncryptionContext<T>)} => {nameof(Decrypt)} completed successfully.");
            }
#pragma warning restore SYSLIB0053 // Type or member is obsolete
        ...
        {
            _logger.LogDebug($"{nameof(EncryptionContext<T>)} => {nameof(Encrypt)} called with plainText: {plainText}");
...
#pragma warning disable SYSLIB0053 // Type or member is obsolete
            using (var aesGcm = new AesGcm(keyBytes))
            {
                ...
                _logger.LogDebug($"{nameof(EncryptionContext<T>)} => {nameof(Encrypt)} completed successfully.");
            }
#pragma warning restore SYSLIB0053 // Type or member is obsolete
